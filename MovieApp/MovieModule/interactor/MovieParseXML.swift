...
class MovieParseXmlImplement: NSObject, MovieParse,XMLParserDelegate
{
    
    
    private var parserCompletionHandler: ((Result<[Movie]>) -> Void)?
    private var rssItems: [Movie] = []
    ...
    func parseData(data: Data, completionHandler: @escaping ((Result<[Movie]>) -> Void)) {
...
//            let rssItem = Movie(title: currentTitle, description: currentDescription.withoutHtml, pubDate: currentPubDate,imgURL: imgURL,link: currentLink,state : .new,imageData : Data())
//            rssItems += [rssItem]
        }
    }
